...
test.describe('Finance Business Partnering Page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/finance-business-partnering');
	});

	test('loads the page successfully', async ({ page }) => {
		await expect(page).toHaveURL('/finance-business-partnering');
		await expect(page).toHaveTitle(/Finance Business Partnering Program/i);
	});

	test.describe('Hero Section', () => {
		test('displays the program branding', async ({ page }) => {
			const logo = page.locator('img[alt="Finance Business Partnering Program"]').first();
			await expect(logo).toBeVisible();

			const brandName = page.getByText('FINANCE PROGRAM', { exact: true }).first();
		...
		test('displays the hero tagline about champions of change', async ({ page }) => {
			const tagline = page.getByText(
				/Champions of change.*Builders of trust.*Leaders in collaboration/i
			);
	...
	test.describe('What Makes This Program Unique Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /WHAT MAKES THIS FINANCE BUSINESS PARTNERING PROGRAM UNIQUE/i
			});
	...
	test.describe('Call to Action Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /THE CALL TO ACTION FOR FINANCE TEAMS/i
			});
			await expect(heading).toBeVisible();
		});

		test('mentions Finance business partners', async ({ page }) => {
			const text = page.getByText('Finance business partners', { exact: false }).first();
	...
	test.describe('Learning Outcomes Section', () => {
		...
		test('shows learning outcomes', async ({ page }) => {
			const outcomes = [
				'Equip themselves with the integral skills',
	...
	test.describe('Core Modules Section', () => {
		...
		test('shows all six module titles', async ({ page }) => {
			const modules = [
				...
				'FINANCE BUSINESS PARTNERING TOOLKIT',
	...
	test.describe('SEO', () => {
		test('has correct meta title', async ({ page }) => {
			await expect(page).toHaveTitle(/Finance Business Partnering Program.*Impactology/i);
		});

		test('has meta description', async ({ page }) => {
			const metaDescription = page.locator('meta[name="description"]');
			await expect(metaDescription).toHaveAttribute('content', /Finance Business Partnering/i);
		});

		test('has Open Graph tags', async ({ page }) => {
			const ogTitle = page.locator('meta[property="og:title"]');
			await expect(ogTitle).toHaveAttribute('content', /Finance Business Partnering Program/i);
