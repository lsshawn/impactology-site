...
test.describe('HR Business Partnering Page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/hr-business-partnering');
	});

	test('loads the page successfully', async ({ page }) => {
		await expect(page).toHaveURL('/hr-business-partnering');
		await expect(page).toHaveTitle(/HR Business Partnering Program/i);
	});

	test.describe('Hero Section', () => {
		test('displays the program branding', async ({ page }) => {
			const logo = page.locator('img[alt="HR Business Partnering Program"]').first();
			await expect(logo).toBeVisible();

			const brandName = page.getByText('HR PROGRAM', { exact: true }).first();
			await expect(brandName).toBeVisible();
		});

		test('shows the main headline', async ({ page }) => {
			const headline = page.getByRole('heading', {
				name: /HR.*PEOPLE.*CULTURE.*BUSINESS PARTNERING IMPACT PROGRAM/i
			});
	...
	test.describe('What Makes This Program Unique Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /WHAT MAKES THIS HR.*PEOPLE.*CULTURE BUSINESS PARTNERING TRAINING UNIQUE/i
			});
	...
	test.describe('Call to Action Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /THE CALL TO ACTION FOR HR.*PEOPLE.*CULTURE TEAMS/i
			});
			await expect(heading).toBeVisible();
		});
	...
	test.describe('Core Modules Section', () => {
		...
		test('shows all six module titles', async ({ page }) => {
			const modules = [
				...
				'HR / PEOPLE & CULTURE BUSINESS PARTNERING TOOLKIT',
	...
	test.describe('SEO', () => {
		test('has correct meta title', async ({ page }) => {
			await expect(page).toHaveTitle(/HR Business Partnering Program.*Impactology/i);
		});
		...
		test('has Open Graph tags', async ({ page }) => {
			...
			await expect(ogTitle).toHaveAttribute('content', /HR Business Partnering Program/i);
