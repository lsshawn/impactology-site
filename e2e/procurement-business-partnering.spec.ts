...
test.describe('Procurement Business Partnering Page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/procurement-business-partnering');
	});

	test('loads the page successfully', async ({ page }) => {
		await expect(page).toHaveURL('/procurement-business-partnering');
		await expect(page).toHaveTitle(/Procurement Business Partnering Program/i);
	});

	test.describe('Hero Section', () => {
		test('displays the program branding', async ({ page }) => {
			const logo = page.locator('img[alt="Procurement Business Partnering Program"]').first();
			await expect(logo).toBeVisible();

			const brandName = page.getByText('PROCUREMENT PROGRAM', { exact: true }).first();
			await expect(brandName).toBeVisible();
		});

		test('shows the main headline', async ({ page }) => {
			const headline = page.getByRole('heading', {
				name: /PROCUREMENT BUSINESS PARTNERING PROGRAM/i
			});
		...
		test('displays the hero tagline about builders of trust', async ({ page }) => {
			const tagline = page.getByText(
				/Builders of trust.*Experts in change.*Champions in collaboration/i
			);
	...
	test.describe('What Makes This Program Unique Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', { name: /WHAT MAKES THIS PROGRAM UNIQUE/i });
			await expect(heading).toBeVisible();
		});

		test('shows the three core principles', async ({ page }) => {
			await expect(page.getByText('Understanding yourself')).toBeVisible();
			await expect(page.getByText('Understanding your business')).toBeVisible();
			await expect(page.getByText('Understanding your stakeholders')).toBeVisible();
		});
	});
	...
	test.describe('Call to Action Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /THE CALL TO ACTION FOR PROCUREMENT TEAMS/i
			});
			await expect(heading).toBeVisible();
		});

		test('mentions procurement business partners', async ({ page }) => {
			const text = page.getByText('procurement business partners', { exact: false }).first();
	...
	test.describe('Core Modules Section', () => {
		...
		test('shows all six module titles', async ({ page }) => {
			const modules = [
				...
				'PROCUREMENT BUSINESS PARTNERING TOOLKIT',
	...
	test.describe('SEO', () => {
		test('has correct meta title', async ({ page }) => {
			await expect(page).toHaveTitle(/Procurement Business Partnering Program.*Impactology/i);
		});
		...
		test('has meta description', async ({ page }) => {
			...
			await expect(metaDescription).toHaveAttribute(
				'content',
				/Procurement Business Partnering Program/i
			);
		});

		test('has Open Graph tags', async ({ page }) => {
			const ogTitle = page.locator('meta[property="og:title"]');
			await expect(ogTitle).toHaveAttribute('content', /Procurement Business Partnering Program/i);
