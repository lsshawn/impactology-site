...
test.describe('Marketing & Communications Business Partnering Program Page', () => {
	test.beforeEach(async ({ page }) => {
		await page.goto('/marketing-communications-business-partnering-program');
	});

	test('loads the page successfully', async ({ page }) => {
		await expect(page).toHaveURL('/marketing-communications-business-partnering-program');
		await expect(page).toHaveTitle(/Marketing.*Communications Business Partnering Program/i);
	});

	test.describe('Hero Section', () => {
		test('displays the program branding', async ({ page }) => {
			const logo = page
				.locator('img[alt="Marketing Communications Business Partnering Program"]')
				.first();
			await expect(logo).toBeVisible();

			const brandName = page.getByText('MARKETING PROGRAM', { exact: true }).first();
			await expect(brandName).toBeVisible();
		});

		test('shows the main headline', async ({ page }) => {
			const headline = page.getByRole('heading', {
				name: /MARKETING.*COMMUNICATIONS TEAM BUSINESS PARTNERING IMPACT PROGRAM/i
			});
	...
	test.describe('Free Strategy Session Section', () => {
		...
		test('shows transform section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /Transform Your Marketing & Communications Teams into Strategic Business Partners/i
			});
	...
	test.describe('Why Choose Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /WHY CHOOSE THE MARKETING.*COMMUNICATIONS BUSINESS PARTNERING IMPACT PROGRAM/i
			});
			await expect(heading).toBeVisible();
		});
		...
		test('shows challenges with solutions', async ({ page }) => {
			await expect(
				page.getByRole('heading', { name: /Siloed Communication and Lack of Collaboration/i })
			).toBeVisible();
			await expect(
				page.getByRole('heading', { name: /Misalignment with Business Strategy/i })
			).toBeVisible();
			await expect(
				page.getByRole('heading', { name: /Difficulty Demonstrating ROI and Value/i })
			).toBeVisible();
			await expect(
				page.getByRole('heading', { name: /Rapid Market Changes and Digital Disruption/i })
			).toBeVisible();
		});
	...
	test.describe('Six Core Modules Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', { name: /SIX CORE MODULES/i });
			await expect(heading).toBeVisible();
		});

		test('shows all module titles', async ({ page }) => {
			const modules = [
				'Understand Your Business',
				'Understand Your Stakeholders',
				'Understand Your Macro World',
				'Your Marketing & Communications Business Partnering Toolkit',
				'Data As A Storytelling Tool',
				'Know & Measure the Value You Deliver'
			];
	...
	test.describe('Program Benefits Section', () => {
		...
		test('shows team benefits', async ({ page }) => {
			await expect(page.getByText('Increased Influence:')).toBeVisible();
			await expect(page.getByText('Enhanced Leadership Skills:')).toBeVisible();
			await expect(page.getByText('Greater Alignment:')).toBeVisible();
		});

		test('shows organisation benefits', async ({ page }) => {
			await expect(page.getByText('Improved Marketing Effectiveness:')).toBeVisible();
			await expect(page.getByText('Greater Efficiency:')).toBeVisible();
			await expect(page.getByText('Stronger Stakeholder Relationships:')).toBeVisible();
		});
	});
	...
	test.describe('Take the First Step Section', () => {
		test('displays the section heading', async ({ page }) => {
			const heading = page.getByRole('heading', {
				name: /TAKE THE FIRST STEP TOWARD MARKETING EXCELLENCE/i
			});
			await expect(heading).toBeVisible();
		});

		test('shows empowerment message', async ({ page }) => {
			const text = page.getByText(/Empower your marketing and communications teams/i);
	...
	test.describe('Register Interest Form', () => {
		...
		test('displays form description', async ({ page }) => {
			const description = page.getByText(/Ready to unlock the full potential/i);
	...
	test.describe('SEO', () => {
		test('has correct meta title', async ({ page }) => {
			await expect(page).toHaveTitle(
				/Marketing.*Communications Business Partnering Program.*Impactology/i
			);
		});
		...
		test('has meta description', async ({ page }) => {
			...
			await expect(metaDescription).toHaveAttribute(
				'content',
				/Empower marketing and communication teams/i
			);
		});
		...
		test('has Open Graph tags', async ({ page }) => {
			...
			await expect(ogTitle).toHaveAttribute(
				'content',
				/Marketing.*Communications Business Partnering Program/i
			);
